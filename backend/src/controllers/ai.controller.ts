...
export const askAssistant = (req: AuthenticatedRequest, res: Response) => {
  ...
  // 2. Search through patient's actual uploaded records if inquiring about personal health
  let matchedRecords: any[] = [];
  if (records && records.length > 0) {
    records.forEach(rec => {
      const text = `${rec.name} ${rec.category} ${rec.summary || ''}`.toLowerCase();
      if (
  ...
  if (matchedRecords.length > 0) {
    const findings = matchedRecords.map(r => `• ${r.name} (${r.date ? new Date(r.date).toLocaleDateString() : 'Recent'}): ${r.summary}`).join('\n');
    ...
    // If asking about personal health results and no records are uploaded yet
    ...
    answer = `I reviewed your vault, which contains ${records.length} document(s): ${recordNames}. Could you specify which lab value, report, or test you would like me to explain from these documents?`;
...
export const getRiskAnalysis = (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id || 'usr-1';
  const user = db.findUserById(userId);
  const records = db.getRecords(userId);
